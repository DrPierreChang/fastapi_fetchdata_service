...
from src.api.v1.resources import predicts, users
...
def startup():
    ...
    cache.access_token_cache = redis_cache.CacheRedis(
        cache_instance=redis.Redis(
            host=config.REDIS_HOST, port=config.REDIS_PORT, max_connections=10, db=1
        )
    )
    cache.refresh_token_cache = redis_cache.CacheRedis(
        cache_instance=redis.Redis(
            host=config.REDIS_HOST, port=config.REDIS_PORT, max_connections=10, db=2, decode_responses=True
        )
    )
...
def shutdown():
    ...
    cache.cache.close()
    cache.access_token_cache.close()
    cache.refresh_token_cache.close()

# Подключаем роутеры к серверу


app.include_router(router=predicts.router, prefix="/api/v1/predicts")
